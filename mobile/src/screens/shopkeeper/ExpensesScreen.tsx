  ...
  return (
    <View style={styles.screen}>
        ...
        <View style={styles.breakdownRow}>
          {categoryBreakdown.slice(0, 3).map(([cat, total]) => (
            <View key={cat} style={styles.breakdownPill}>
              <View style={[styles.breakdownDot, { backgroundColor: CATEGORY_COLORS[cat] || '#64748B' }]} />
      ...
      <FlatList
        ...
        ListEmptyComponent={
          <Text style={styles.empty}>No expenses for this month. Tap + Add to record one.</Text>
        }
        renderItem={({ item }) => (
          <Pressable
            style={styles.row}
...
const styles = StyleSheet.create({
  ...
  monthArrow: { color: '#FFF', fontSize: 28, fontWeight: '300', lineHeight: 30 },
  monthLabel: { color: '#FFF', fontSize: t.bodyLg, fontWeight: '700' },
  totalCard: {
    marginTop: 12,
  ...
  totalValue: { color: '#FFF', fontSize: t.xl, fontWeight: '800', marginTop: 4 },
  ...
  breakdownRow: { flexDirection: 'row', flexWrap: 'wrap', gap: 8, paddingHorizontal: 16, marginBottom: 4 },
  breakdownPill: {
    flexDirection: 'row',
    alignItems: 'center',
    gap: 6,
  ...
  rowNote: { fontSize: t.sm, color: colors.textMuted, marginTop: 4 },
  rowAmount: { fontSize: t.bodyLg, fontWeight: '800', color: '#2563EB' },
